...
def put_text_unicode(image, transcirpt, pos, font_size=int(20 * 0.5), color=(255, 255, 255)):
    # cv2 to pil
    cv2img = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    pilimg = Image.fromarray(cv2img)

    draw = ImageDraw.Draw(pilimg)
    font = ImageFont.truetype(os.path.abspath('modules/utils/fonts/font.ttf'), font_size)
    draw.text(pos, transcirpt, color, font=font)

    # pil to cv2 img
    cv2out = cv2.cvtColor(np.array(pilimg), cv2.COLOR_RGB2BGR)
...
def show_box(image, box, transcirpt):
    ...
    img = put_text_unicode(image, transcirpt, (origin[0], origin[1] - 15), int(20 * 0.5), (255, 255, 0))
    img = cv2.polylines(img, [pts], True, [150, 200, 200], 2)
        ...
        with open(res_file, 'w') as f:
            for i, box in enumerate(boxes):
                ...
                f.write('{0},{1},{2},{3},{4},{5},{6},{7},{8:.2f},{9}\r\n'.format(
                    ...
                    scores[i], pred_transcripts[i]
                ))

                # f.write('{0},{1},{2},{3},{4},{5},{6},{7},{8:.2f}\r\n'.format(
                #     box[0, 0], box[0, 1], box[1, 0], box[1, 1], box[2, 0], box[2, 1], box[3, 0], box[3, 1],
                #     scores[i]
                # ))
                ...
                if with_img:
                    origin = poly[0]
                    img = put_text_unicode(img, pred_transcripts[i], (origin[0], origin[1] - 15))
                    img = cv2.polylines(img, [box.astype(np.int32).reshape((-1, 1, 2))], True, color=(255, 255, 0),
                                        thickness=1)
