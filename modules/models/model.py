...
# from modules.models.core.resnet import ResNetBackbone
# from modules.models.core.mobilenet import MobileNetV3_Backbone
...
class OCRModel:
    ...
    def __init__(self, config):
        ...
        # assert backbone_name in ['mobilenet', 'resnet'], 'Backbone not found! please use [mobilenet] or [resnet]'
        # if backbone_name == 'mobilenet':
        #     self.backbone = MobileNetV3_Backbone(config)
        #     backbone_channel_out = 128
        # elif backbone_name == 'resnet':
        #     resnet_backbone = pm.__dict__['resnet50'](pretrained='imagenet')  # resnet50 in paper
        #     self.backbone = ResNetBackbone(resnet_backbone, config)
        self.backbone = ResNetBackbone(config)

        num_class = len(keys) + 1
        self.recognizer = Recognizer(num_class, config)
        ocr_pretrained = config['model']['ocr_pretrain']
        if ocr_pretrained and os.path.exists(ocr_pretrained):
            print('Load recognition pretrained model from [{}]'.format(ocr_pretrained))
            self.recognizer.load_state_dict(torch.load(ocr_pretrained, map_location='cpu'))
